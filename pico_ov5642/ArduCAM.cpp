/*
  ...
  release. I can be contacted through http://www.ArduCAM.com
...
/*------------------------------------
  ...
  2012/10/23  V1.0.1  by Lee  Resolved some timing issue for the Read/Write
  Register 2012/11/29  V1.1.0  by Lee  Add support for MT9D111 sensor 2012/12/13
  V1.2.0  by Lee  Add support for OV7675 sensor 2012/12/28  V1.3.0  by Lee  Add
  support for OV2640,OV3640,OV5642 sensors 2013/02/26  V2.0.0  by Lee  New Rev.B
  shield hardware, add support for FIFO control and support Mega1280/2560 boards
  2013/05/28  V2.1.0  by Lee  Add support all drawing functions derived from
  UTFT library 2013/08/24  V3.0.0  by Lee  Support ArudCAM shield Rev.C
  hardware, features SPI interface and low power mode. Support almost all series
  of Arduino boards including DUE. 2014/02/06  V3.0.1  by Lee  Minor change to
  the library, fixed some bugs, add self test code to the sketches for easy
  debugging. 2014/03/09  V3.1.0  by Lee  Add the more impressive example
  sketches. Optimise the OV5642 settings, improve image quality. Add live
  preview before JPEG capture. Add play back photos one by one after BMP
  capture. 2014/05/01  V3.1.1  by Lee  Minor changes to add support Arduino IDE
  for linux distributions. 2014/09/30  V3.2.0  by Lee  Improvement on OV5642
  camera dirver. 2014/10/06  V3.3.0  by Lee  Add OV7660,OV7725 camera support.
  2015/02/27  V3.4.0  by Lee  Add the support for Arduino Yun board, update the
  latest UTFT library for ArduCAM. 2015/06/09  V3.4.1  by Lee  Minor changes and
  add some comments 2015/06/19  V3.4.2  by Lee  Add support for MT9M112 camera.
  ...
  2016/10/05  V4.0.0  by Lee	Add support for second generation hardware
  platforms like ArduCAM shield V2, ArduCAM-Mini-5MP-Plus(OV5642/OV5640).
...
ArduCAM::ArduCAM(byte model, int CS) {
  ...
  if (CS >= 0) {
    B_CS = CS;
     ...
     defined(NRF52840_XXAA) || defined(ARDUINO_ARCH_RP2040))
  B_CS = CS;
#else
...
void ArduCAM::flush_fifo(void) { write_reg(ARDUCHIP_FIFO, FIFO_CLEAR_MASK); }

void ArduCAM::start_capture(void) { write_reg(ARDUCHIP_FIFO, FIFO_START_MASK); }
...
void ArduCAM::CS_HIGH(void) { sbi(P_CS, B_CS); }
void ArduCAM::CS_LOW(void) { cbi(P_CS, B_CS); }
...
void ArduCAM::OV5642_set_RAW_size(uint8_t size) {
    ...
    defined(OV5642_MINI_5MP) || defined(OV5642_MINI_5MP_PLUS)
  switch (size) {
  case OV5642_640x480:
    wrSensorRegs16_8(OV5642_1280x960_RAW);
    wrSensorRegs16_8(OV5642_640x480_RAW);
...
int ArduCAM::wrSensorRegs8_8(const struct sensor_reg reglist[]) {
...
#else
  int err = 0;
  uint8_t reg_addr = 0; // Use uint8_t for 8-bit addresses
  uint16_t reg_val = 0;
  int count = 0;
  ...
  while (true) {
    ...
    if (reg_addr == 0xff && reg_val == 0xffff)
      break;
    err = wrSensorReg8_8(reg_addr, reg_val);
    if (err == 0) {
      Serial.print(F("ACK CMD I2C ERR 8_8: 0x"));
  ...
  Serial.print(F("ACK CMD wrSensorRegs8_8 count: "));
  Serial.print(count);
  Serial.println(F(" END"));
#endif
  return 1;
...
int ArduCAM::wrSensorRegs8_16(const struct sensor_reg reglist[]) {
  ...
  Serial.print(F("ACK CMD wrSensorRegs8_16 count: "));
  Serial.print(count);
  Serial.println(F(" END"));
#endif
  return 1;
...
int ArduCAM::wrSensorRegs16_8(const struct sensor_reg reglist[]) {
...
#else
  int err = 0;
  uint16_t reg_addr = 0; // Standardized to 16-bit
  ...
  while (true) {
    ...
    if (err == 0) {
      Serial.print(F("ACK CMD I2C ERR 16_8: 0x"));
      Serial.print(reg_addr, HEX);
      Serial.println(F(" END"));
    }
    next++;
  ...
  Serial.print(F("ACK CMD wrSensorRegs16_8 count: "));
  Serial.print(count);
  Serial.println(F(" END"));
#endif
  return 1;
...
int ArduCAM::wrSensorRegs16_16(const struct sensor_reg reglist[]) {
  ...
  while (true) {
    reg_addr = pgm_read_word(&next->reg);
    reg_val = pgm_read_word(&next->val);
    if (reg_addr == 0xffff && reg_val == 0xffff)
      break;
    err = wrSensorReg16_16(reg_addr, reg_val);
    if (err == 0) {
      Serial.print(F("ACK CMD I2C ERR 16_16: 0x"));
      Serial.print(reg_addr, HEX);
      Serial.println(F(" END"));
    }
    next++;
  ...
  Serial.print(F("ACK CMD wrSensorRegs16_16 count: "));
  Serial.print(count);
  Serial.println(F(" END"));
#endif
  return 1;
...
byte ArduCAM::wrSensorReg16_8(int regID, int regDat) {
  ...
  if (error) {
    Serial.print(F("ACK CMD I2C WRITE ERROR: 0x"));
...
byte ArduCAM::rdSensorReg16_8(uint16_t regID, uint8_t *regDat) {
  ...
  byte error = Wire.endTransmission();
  if (error) {
    Serial.print(F("ACK CMD I2C READ ADDR ERROR Code: "));
    Serial.print(error);
    Serial.println(F(" END"));
    return 0;
  }
  ...
  if (Wire.available()) {
    *regDat = Wire.read();
    ...
    Serial.println(F("ACK CMD I2C READ DATA ERROR END"));
    return 0;
  }
#endif
  return 1;
